...
if e=='y'
    exp.sneff=sn-Pf; %effective normal stress in MPa
else
    exp.sneff=sn;  % if Pf = 0, effective normal stress = normal stress   
end
...
% initialState = setupFcn(gcf);     %Call the crosshair
...
getcoordmouse                     %Display the x-y coordinates of the mouse in the plot
...
for i=1:N/2                      %number of velocity steps
    ...
    %define the time,slip,friction, and effective normal stress arrays corresponding to the selected velocity step 
    ...
    %check files
    rawdata= dlmread(text,'\t',1,0); %rawdata is also the output of the velocity steps in the Matlab Workspace                                 
    figure(); %check graphycally if you have sliced the velocity steps correctly
...
ax=gca; ax.XAxis.Exponent = 0;
set(h, 'ButtonDownFcn',@lineClickedCallback); %Select the point: fires when mouse is pressed.
...
%            % Code from the standard 'Update Function'
            ...
            output_txt = {['Step: ',num2str(number)],num2str(k)}; %Display number of velocity step
        end
